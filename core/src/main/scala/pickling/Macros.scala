...
trait PicklerMacros extends Macro {
  ...
  def impl[T: c.WeakTypeTag](format: c.Tree): c.Tree = preferringAlternativeImplicits {
    ...
    val picklerName = picklerClassName.toTermName
    val typeName = tpe.toString
    val registryName = c.fresh(newTermName("registry"))

    /* also generate unpickler class, so that we can
       derive its name from the pickler class name */
    val unpicklerClassNameString = "Pickler".r.replaceFirstIn(picklerClassNameString, "Unpickler")
    val unpicklerClassName = newTypeName(unpicklerClassNameString)
    val c0: c.type = c
    val bundle = new { val c: c0.type = c0 } with UnpicklerMacros
    val unpicklerTree = bundle.impl[T](format, unpicklerClassName)
...
trait UnpicklerMacros extends Macro {
  def impl[T: c.WeakTypeTag](format: c.Tree): c.Tree = {
    ...
    val name = syntheticUnpicklerName(tpe)
    val unpicklerClassName = c.fresh(name)
    impl[T](format, unpicklerClassName)
  }

  def impl[T: c.WeakTypeTag](format: c.Tree, unpicklerClassName: c.TypeName): c.Tree = preferringAlternativeImplicits {
    import c.universe._
    import compat._
    import definitions._
    val tpe = weakTypeOf[T]
    ...
    val unpicklerName = unpicklerClassName.toTermName
    q"""
      implicit var $unpicklerName: scala.pickling.Unpickler[$tpe] with Generated = null
      class $unpicklerClassName extends scala.pickling.Unpickler[$tpe] with Generated {
        import scala.pickling._
      ...
      $unpicklerName = new $unpicklerClassName
      $unpicklerName
    """
  }
}
