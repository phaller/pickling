...
trait PicklerUnpicklerMacros extends Macro
  ...
  override def impl[T: c.WeakTypeTag]: c.Tree = preferringAlternativeImplicits {
    ...
    val createTagTree = super[StaticTypeTagMacros].impl[T]
 ...
 */
trait OpenSumUnpicklerMacro extends Macro with UnpicklerMacros with StaticTypeTagMacros {
  override def impl[T: c.WeakTypeTag]: c.Tree = preferringAlternativeImplicits {
    import c.universe._
    import compat._
    import definitions._
...
// but not its subclasses or the types convertible to it!
trait UnpicklerMacros extends Macro with UnpickleMacros with StaticTypeTagMacros {
...
trait UnpickleMacros extends Macro with TypeAnalysis {
  ...
  def createRefDispatch(): CaseDef =
    CaseDef(Literal(Constant(StaticTypeTag.Ref.key)), EmptyTree, createUnpickler(typeOf[refs.Ref]))

  def createNullDispatch(): CaseDef =
    CaseDef(Literal(Constant(StaticTypeTag.Null.key)), EmptyTree, createUnpickler(typeOf[Null]))
