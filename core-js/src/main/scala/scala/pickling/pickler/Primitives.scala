...
class PrimitivePickler[T: StaticTypeTag](name: String) extends AutoRegister[T](name) {
...
object PrimitivePickler {
  def apply[A: StaticTypeTag]: Pickler[A] with Unpickler[A] =
    new PrimitivePickler[A](StaticTypeTag.valueTypeName(implicitly[StaticTypeTag[A]]))
}
